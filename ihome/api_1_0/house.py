...
def show_areas_info():
    ...
    try:
        ...
        areas = None
    # 2 判断获取结果,如果有数据,留下访问redis数据库的记录,返回数据给前端
    if areas:
        current_app.logger.info('hit redis areas info')
        # 因为redis中存储的就是json格式的字符串,可以直接返回
        return '{"errno":0, "errmsg":"OK", "data":%s}' % areas
    # 3 如果缓存中没有数据,需要读取mysql数据库,然后将数据存入缓存中
    try:
    ...
    # 4 校验从数据库中查询的城区信息
    if not areas:
        # 如果数据库中没有查询到相关信息,直接返回
        return jsonify(errno=RET.NODATA, errmsg="无城区信息")
    # 5 如果有数据,将数据缓存到redis中
    # 5.1 定义容器存储查询结果
    areas_list = []
    # 5.2 遍历查询结果
    for area in areas:
        # 5.3 调用area对象的to_dict()方法,返回城区id和name
...
def set_house_image(house_id):
    ...
    4/读取图片数据
    5/调用七牛云接口上传图片,保存返回的图片信息
    6/构造模型类对象,HouseImage对象,存储图片关联的房屋,以及图片的名称
    7/临时提交数据到HouseImage对象
    8/判断房屋主图片是否设置,如未设置则默认添加当前图片为房屋主图片
    9/提交临时对象到House对象
    10/提交数据到数据库中,如果发生异常需要回滚
    11/拼接图片的绝对路径
    12/返回结果
    :param house_id:
    :return:
    ...
    # 校验查询结果
    if not house:
        return jsonify(error=RET.NODATA, errmsg='无房屋数据')
    # 读取图片数据
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(error=RET.THIRDERR, errmsg='上传到七牛云发生异常')
    # 构造模型类对象,准备存储房屋图片数据
    ...
    # 判断房屋主图片是否设置,如果没有设置,则将当前图片设置为房屋主图片
    ...
    # 返回结果
    return jsonify(errno=0, errmsg='OK', data={'url':image_url})
...
def get_houses_index():
    ...
    4/查询mysql数据库,对幻灯片的处理,默认采取房屋的成交次数,最多显示五条
    5/判断获取结果
    6/定义容器,遍历获取结果,判断房屋是否设置图片,如果未设置默认不添加
    7/序列化房屋数据
    8/存入redis数据库中
    9/返回结果
    :return:
...
def get_house_detail(house_id):
    """
    获取房屋详情信息
    1/首先尝试获取用户的身份   user_id = session.get('user_id', '-1')
    2/校验house_id参数的存在
    ...
    if ret:
        ...
        return '{"errno":RET.OK, "errmsg":"OK", "data":{"user_id":%s, "house":%s}}' % (user_id, ret)
    # 如果redis缓存中没有读取到数据,需要查询mysql数据库
    try:
        house = House.query.get(house_id)
    except Exception as e:
        current_app.logger.error(e)
    ...
    # 获取房屋详情数据
    try:
        # 因为to_full_dict()方法里面实现房屋详情数据,需要查询数据库,所以需要进行异常捕获
    ...
    # 把房屋详情数据存入redis缓存中
    try:
        redis_store.setex('house_info_%s' % house_id, constants.HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS, house_json)
    except Exception as e:
        current_app.logger.error(e)
    # 构造响应数据
...
def get_houses_list():
    ...
    5/尝试从redis缓存中获取房屋的列表信息,使用的是哈希数据类型
    ...
    9/判断区域参数的是否存在,如果存在添加到过滤条件中
    10/判断日期参数是否存在,比较用户选择的日期和数据库中订单的日期,得到满足条件的房屋
    11/过滤条件的容器内已经存储了区域信息,和满足条件的房屋
    ...
    houses_list = houses.items
    total_page = houses_page.pages
    14/b遍历房屋数据,调用模型类中的方法,获取房屋的基本数据
    15/构造响应报文
    resp = {
    ...
    18/构造redis_key,存储房屋列表页的缓存数据,因为使用的是hash数据类型,为了确保数据的完整性
    需要使用事务:开始事务,存储数据,设置有效期,执行事务
    ...
    # 尝试从redis缓存中获取房屋的列表数据, 因为多条数据的存储, 使用hash数据类型,首先需要键值
    try:
        # redis_key相当于hash对象,里面存储的是页数和对应的房屋数据
        redis_key = 'houses_%s_%s_%s_%s'%(area_id, start_date_str, end_date_str, sort_key)
        # 根据redis_key获取缓存数据
